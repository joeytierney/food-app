"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var core_1 = require('@angular/core');
var cuisine_1 = require('../cuisine/cuisine');
var CuisinesService = (function () {
    function CuisinesService() {
    }
    CuisinesService.prototype.getCuisines = function () {
        return [
            new cuisine_1.Cuisine(1, 'Asian'),
            new cuisine_1.Cuisine(2, 'Chinese'),
            new cuisine_1.Cuisine(3, 'European'),
            new cuisine_1.Cuisine(4, 'French'),
            new cuisine_1.Cuisine(5, 'Indian'),
            new cuisine_1.Cuisine(6, 'Irish')
        ];
    };
    CuisinesService = __decorate([
        core_1.Injectable(), 
        __metadata('design:paramtypes', [])
    ], CuisinesService);
    return CuisinesService;
}());
exports.CuisinesService = CuisinesService;
//# sourceMappingURL=cuisines-service.js.map